...
public class GroupServiceImpl implements GroupService {
    ...
    private final AcademicPeriodMongoRepository periodRepository; // Necesitas este repositorio

    public GroupServiceImpl(
            GroupMongoRepository groupRepository,
            SubjectMongoRepository subjectRepository,
            AcademicPeriodMongoRepository periodRepository) {
    ...
    public List<Group> findAll() {
        ...
        try {
            ...
            log.error("Error al obtener todos los grupos", e);
    ...
    @Transactional
    @Override
    public Group findByCode(String code) throws SirhaException {
        try {
            if (code == null) {
                log.error("Error: Código del grupo no puede ser null");
                throw SirhaException.of(ErrorCodeSirha.GROUP_NOT_FOUND, "El código del grupo no puede ser null");
            }
            log.info("Buscando grupo con código: {}", code);
            Group group = groupRepository.findByCode(code)
                    .orElseThrow(() -> {
                        log.warn("Grupo con código {} no encontrado", code);
                        return SirhaException.of(ErrorCodeSirha.GROUP_NOT_FOUND, "Grupo con código " + code + " no encontrado");
                    });
            log.info("Grupo encontrado: {}", group);
            return group;
        } catch (SirhaException e) {
    ...
    @Transactional
    @Override
    public Group save(GroupDTO groupDTO, String subjectName) throws SirhaException {
        try {
            if (groupDTO == null) {
                log.error("Error: El DTO del grupo no puede ser null");
                throw SirhaException.of(ErrorCodeSirha.INVALID_ARGUMENT, "El grupo no puede ser null");
            }
            ...
            log.info("Guardando grupo para la materia: {}", subjectName);

            // Buscar la materia
            Subject subject = subjectRepository.findByName(subjectName)
                    .orElseThrow(() -> SirhaException.of(ErrorCodeSirha.SUBJECT_NOT_FOUND, "Materia no encontrada: " + subjectName));

            // Buscar el periodo académico
            AcademicPeriod period = periodRepository.findById(groupDTO.getAcademicPeriodId())
                    .orElseThrow(() -> SirhaException.of(ErrorCodeSirha.INVALID_ARGUMENT, "Periodo académico no encontrado"));

            // Crear el grupo
            Group group = new Group(subject, groupDTO.getCapacidad(), period);

            if (groupDTO.getAula() != null) {
                group.setAula(groupDTO.getAula());
            }

            Group savedGroup = groupRepository.save(group);

            // Actualizar la materia con el nuevo grupo
    ...
    @Transactional
    @Override
    public Group deleteById(String id) throws SirhaException {
    ...
    @Transactional
    @Override
    public List<Group> findBySubjectName(String subjectName) throws SirhaException {
        log.info("Buscando grupos por nombre de materia: {}", subjectName);
        try {
            Subject subject = subjectRepository.findByName(subjectName)
                    .orElseThrow(() -> SirhaException.of(ErrorCodeSirha.SUBJECT_NOT_FOUND, "Materia no encontrada: " + subjectName));

            List<Group> groups = subject.getGroups();
            log.info("Total de grupos encontrados para materia '{}': {}", subjectName, groups.size());
            return groups;
        } catch (SirhaException e) {
            throw e;
        } catch (Exception e) {
            throw SirhaException.of(ErrorCodeSirha.INTERNAL_ERROR, "Error interno al buscar grupos por materia: " + e.getMessage(), e);
        }
    }

    @Transactional
    @Override
    public List<Group> findOpenGroupsBySubjectName(String subjectName) throws SirhaException {
        log.info("Buscando grupos abiertos por nombre de materia: {}", subjectName);
        try {
            Subject subject = subjectRepository.findByName(subjectName)
                    .orElseThrow(() -> SirhaException.of(ErrorCodeSirha.SUBJECT_NOT_FOUND, "Materia no encontrada: " + subjectName));

            List<Group> openGroups = subject.getOpenGroups();
            log.info("Total de grupos abiertos encontrados para materia '{}': {}", subjectName, openGroups.size());
            return openGroups;
        } catch (SirhaException e) {
            throw e;
        } catch (Exception e) {
            throw SirhaException.of(ErrorCodeSirha.INTERNAL_ERROR, "Error interno al buscar grupos abiertos por materia: " + e.getMessage(), e);
        }
    }

    @Transactional
    @Override
    public List<Group> deleteBySubjectName(String subjectName) throws SirhaException {
        ...
        try {
            ...
            Subject subject = subjectRepository.findByName(subjectName)
                    .orElseThrow(() -> {
                        ...
                        return SirhaException.of(ErrorCodeSirha.SUBJECT_NOT_FOUND, "Materia no encontrada: " + subjectName);
                    });
            ...
            // Limpiar la lista de grupos en el subject
